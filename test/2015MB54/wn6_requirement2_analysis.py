"""
2025 WN6 双飞行器约束分析（依据 2025WN6-要求2.md）。

- /celestial/mpc: 小行星根数 + 星历
- /celestial/transfer: Earth -> 2025 WN6 Lambert 网格
- /celestial/ephemeris: 到达时刻日心地球位置，用于地-星距离
"""
...
DEPARTURE_INTERVAL = "2028-10-01T00:00:00Z/2030-05-31T00:00:00Z"
ARRIVAL_INTERVAL = "2029-05-01T00:00:00Z/2030-08-31T00:00:00Z"
STEP_DAYS = 5
MIN_TOF_DAYS = 10
...
IMPACTOR_DIST_MAX_KM = 20_000_000.0  # 撞击器到达时地-星距离上限（本次按需求放宽为 2000 万 km）
IMPACTOR_DEP_DV_MAX_MPS = 1_100.0  # 1.1 km/s

OBSERVER_DV_TOTAL_MAX_MPS = 2_000.0  # dV_earth + DV2_Mag, 合不超过 2 km/s
MIN_GAP_DAYS = 90

SUN_FRAME = "MeanEclpJ2000"

EARTH_EPHEM_START = "2029-05-01T00:00:00Z"
EARTH_EPHEM_STOP = "2030-08-31T00:00:00Z"
...
def departure_dv_metric_mps(dv1_mag_mps: float) -> float:
    """双曲超速 DV1_Mag -> 200 km 圆轨道高度近地点速度 - 10.96 km/s（文档定义）。"""
    v_inf = dv1_mag_mps
...
def mpc_orbit_to_transfer_elements(orbit: dict) -> dict:
    """将 /celestial/mpc 返回的 OrbitElements 转为 /celestial/transfer 所需字段（单位与示例 fixture 一致）。"""
    keys = [
        "EpochMjdTdt",
        "SemimajorAxis",
        "Eccentricity",
    ...
    # MPC 文档写 SemimajorAxis 为 m；若数值极大则换算为 AU
    au = 149597870700.0
...
def main() -> None:
    ...
    save_json(OUT_DIR / "mpc-2025WN6-input.json", mpc_in)
    mpc_out = post_json("/celestial/mpc", mpc_in, timeout=120)
    save_json(OUT_DIR / "mpc-2025WN6-output.json", mpc_out)
    ...
    xfer_out = post_json("/celestial/transfer", xfer_in, timeout=600)
    ...
    earth_out = post_json("/celestial/ephemeris", earth_in, timeout=180)
    ...
    impactors: list[Cand] = []
    observers: list[Cand] = []
    # 撞击器约束逐项累计（用于说明无可行解的原因）
    n_imp_illum = n_imp_dv2 = n_imp_dist = n_imp_dep = 0
    min_dep_among_triple: float | None = None  # 光照+DV2+距离均已满足时的最小 |dV|
    for c in enriched:
        dd = abs(c.dep_dv_mps)
    ...
    lines = [
        ...
        "## 1. 输入与接口",
        "",
        "- **本次调整**: 撞击器到达时地-星距离上限已放宽为 **2000 万 km**（脚本常量 `IMPACTOR_DIST_MAX_KM`），其余约束不变。",
        "",
        f"- 地球出发搜索区间: `{DEPARTURE_INTERVAL}`",
        f"- 小行星到达搜索区间: `{ARRIVAL_INTERVAL}`",
        f"- Lambert 网格步长: {STEP_DAYS} 天（出发/到达相同）",
        f"- 转移请求与响应: `celestial-transfer-input.json`, `celestial-transfer-output.json`",
        f"- MPC: `mpc-2025WN6-input.json`, `mpc-2025WN6-output.json`",
        f"- 地球星历（用于到达时地-星距离）: `earth-ephemeris-input.json`, `earth-ephemeris-output.json`",
        "",
        "## 2. 物理量定义",
        "",
        "- **到达光照角**: `RV2` 前 3 元（小行星日心位置）与 `DeltaV2` 的夹角（0°–180°）。",
        "- **地球出发 dV（与 10.96 km/s 比较）**: 将 `DV1_Mag` 视为双曲超速 \\(v_\\infty\\)，",
        f"  近地点半径取 200 km（{RP_EARTH_200KM_M:.0f} m），",
        ...
        f"- 到达时地-星距离 < {IMPACTOR_DIST_MAX_KM/1e6:g} Mkm",
        f"- `|dV|` ≤ {IMPACTOR_DEP_DV_MAX_MPS/1000:g} km/s",
        "",
        "**观测器**",
        f"- `|dV| + DV2_Mag` ≤ {OBSERVER_DV_TOTAL_MAX_MPS/1000:g} km/s（两项均为 m/s，合计不超过 2 km/s）",
        f"- 到达时刻比撞击器早 ≥ {MIN_GAP_DAYS} 天",
        "",
        "## 4. 筛选统计",
        "",
    ...
    if pair_records:
        ...
        if min_dep_among_triple is not None:
            lines.append(
                f"在已满足前 **三条** 撞击器条件的样本中，`|dV|` 的**最小值**仍约为 **{min_dep_among_triple/1000:.3f} km/s**，"
                f"高于 1.1 km/s 上限，因此**撞击器集合为空**，无法与观测器配对。"
            )
        lines.extend(
            [
                "",
                f"- 观测器（`|dV|+DV2_Mag ≤ 2 km/s`）可行样本数: {len(observers)}",
