...
- 在 Access 结果上增加约束：上海处于黑夜（`Umbra`），空间站处于光照（`SunLight`）。
- 保存所有 Web API 请求与响应 JSON 文件，以及最终约束筛选结果 JSON。
...
3. 通过 `POST /access/AccessComputeV2` 计算上海到天宫的 Access 区间。
4. 通过 `POST /Lighting/LightingTimes` 分别计算：
   - 上海 `Umbra`（黑夜）区间；
   - 天宫 `SunLight`（光照）区间。
5. 进行区间求交：`Access ∩ Shanghai.Umbra ∩ Tiangong.SunLight`，得到最终有效区间。
6. 生成 `outputs/final_access_with_light_constraints.json`，输出明细与统计。

## 验证标准
- API 返回满足 HTTP 200 且响应 JSON `IsSuccess=true`（在服务可用时）。
- 输出目录包含所有 `request_*.json`、`response_*.json` 和最终结果文件。
- 最终结果至少包含字段：`window`、`input_sources`、`raw_counts`、`constrained_passes`、`summary`。
