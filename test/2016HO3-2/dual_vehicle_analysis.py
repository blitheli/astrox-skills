"""
2016 HO3 双飞行器约束分析（依据 test/2016HO3-2/2016HO3要求.md）。
...
DEPARTURE_INTERVAL = "2028-10-01T00:00:00Z/2032-05-31T00:00:00Z"
ARRIVAL_INTERVAL = "2029-05-01T00:00:00Z/2032-08-31T00:00:00Z"
STEP_DAYS = 5
MIN_TOF_DAYS = 10
...
IMPACTOR_DIST_MAX_KM = 20_000_000.0  # 2000 万 km（文档规定）
...
EARTH_EPHEM_STOP = "2032-08-31T00:00:00Z"
...
def main() -> None:
    ...
    obs_diag: dict = {}
    if enriched:
        costs = [abs(c.dep_dv_mps) + c.dv2_mag for c in enriched]
        obs_diag = {
            "observer_cost_definition": "|dV| + DV2_Mag (m/s)",
            "grid_min_cost_mps": min(costs),
            "grid_count_le_2000_mps": sum(1 for x in costs if x <= OBSERVER_DV_TOTAL_MAX_MPS),
            "observer_DV_total_limit_mps": OBSERVER_DV_TOTAL_MAX_MPS,
        }
        save_json(OUT_DIR / "analysis-observer-diagnostic.json", obs_diag)
    ...
    if pair_records:
        ...
        if obs_diag and len(observers) == 0:
            lines.extend(
                [
                    "",
                    ...
                    "",
                    "```json",
                    json.dumps(obs_diag, ensure_ascii=False, indent=2),
                    "```",
                    "",
