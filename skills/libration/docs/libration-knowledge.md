...
| 接口 | 族参数 | 线上数据范围(以 `IsSuccess` 为准) |
| --- | --- | --- |
| `em-l1-halo` | `Az=X0[2]` | 约 `0.022-0.199` |
| `em-l2-halo` | `Ax=X0[0]-1` | 约 `[0.026000000000018453, 0.1928]` |

`isSouth=true` 通过翻转 z 分量生成南半球分支.

NRHO(Near Rectilinear Halo Orbit)是高振幅 Halo 族靠近月球端的部分,具有近月段速度快、远月段停留久的特征. OpenAPI 同时写了族范围约 `0.05-0.1928` 与典型 NRHO `Ax=0.026`;线上实际下限约为 `0.026000000000018453`,因此字面量 `ax=0.026` 会被拒绝,应使用略大的值(例如 `0.0261`). 调用时必须以线上响应的 `IsSuccess` 为准.
