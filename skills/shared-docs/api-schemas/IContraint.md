...
| ---------------- | -------------------------- | ------------------------ |
| `Range`          | `ConstraintRange`          | 距离(斜距)上下限约束,单位 km        |
| `AzElMask`       | `ConstraintAzElMask`       | 方位角–仰角遮罩(成对采样点),角度单位 rad |
| `ElevationAngle` | `ConstraintElevationAngle` | 仰角上下限约束,单位 deg           |
...
**Schema**:`ContraintConstraintRange`
...
**Schema**:`ContraintConstraintAzElMask`
...
**Schema**:`ContraintConstraintElevationAngle`
...
`EntityPath` 的 `Constraints` 为上述对象的数组,可混合多种 `$type`,例如:
