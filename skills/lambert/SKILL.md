---
name: lambert
description: 求解 Lambert 问题(始末位置速度已知,单圈转移),输出起点和终点的速度增量(DV1、DV2)。当用户需要根据始末状态向量和飞行时间计算轨道转移速度增量时使用。
---
...
2. **单位确认**:位置分量单位为 m,速度分量单位为 m/s,飞行时间单位为 s。
3. **中心天体**:默认地球引力常数 `Gm = 3.986004415E14 m^3/s^2`,其他天体需显式传入。
4. **API 调用逻辑**:向 `{BASE_URL}/orbit/lambert` 发送 `POST`,`Content-Type: application/json`。
...
| 参数名  | 类型     | 必须 | 说明                                                                                   |
| ------- | -------- | ---- | -------------------------------------------------------------------------------------- |
| `RV1`   | number[] | 是   | 初始位置速度:前 3 元为位置(m),后 3 元为速度(m/s),数组长度至少 6                  |
| `RV2`   | number[] | 是   | 末态位置速度:前 3 元为位置(m),后 3 元为速度(m/s),数组长度至少 6                  |
| `TOF`   | number   | 是   | 飞行时间(s)                                                                           |
