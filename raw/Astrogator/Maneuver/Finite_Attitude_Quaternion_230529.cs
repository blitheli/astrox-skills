...
public partial class ManeuverFiniteTests
{
    ...
    */
    [TestMethod()]
    public void Finite_Attitude_Quaternion_230529()
    {
        //  输入json文件的路径
        ...
        //  读取输入参数(json)
        string fp = Path.Combine(filePath0, "Finite_Attitude_Quaternion_230529.json");
        ...
        Assert.AreEqual(5623112.8347527637743, output.Position.cartesianVelocity[id - 6], 3e-5);
        Assert.AreEqual(3196457.4624350448175, output.Position.cartesianVelocity[id - 5], 3e-5);
        Assert.AreEqual(1796483.1639633337090, output.Position.cartesianVelocity[id - 4], 3e-5);
        Assert.AreEqual(-4096.4845640754568, output.Position.cartesianVelocity[id - 3], 3e-8);
        Assert.AreEqual(5650.4624829640848, output.Position.cartesianVelocity[id - 2], 3e-8);
        Assert.AreEqual(3309.1550293822420, output.Position.cartesianVelocity[id - 1], 3e-8);
        //  推进剂质量
