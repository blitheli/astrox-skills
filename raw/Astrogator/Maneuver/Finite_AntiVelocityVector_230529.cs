...
public partial class ManeuverFiniteTests
{
    ...
    */
    [TestMethod()]
    public void Finite_AntiVelocityVector_230529()
    {
        //  输入json文件的路径
        ...
        //  读取输入参数(json)
        string fp = Path.Combine(filePath0, "Finite_AntiVelocityVector_230529.json");
        ...
        Assert.AreEqual(5602895.3733118296441, output.Position.cartesianVelocity[id - 6], 3e-5);
        Assert.AreEqual(3154809.0437089063016, output.Position.cartesianVelocity[id - 5], 3e-5);
        Assert.AreEqual(1712921.5515526370837, output.Position.cartesianVelocity[id - 4], 3e-5);
        Assert.AreEqual(-4161.1447166162456, output.Position.cartesianVelocity[id - 3], 3e-8);
        Assert.AreEqual(5476.0861116620836, output.Position.cartesianVelocity[id - 2], 3e-8);
        Assert.AreEqual(2973.2721660378139, output.Position.cartesianVelocity[id - 1], 3e-8);
