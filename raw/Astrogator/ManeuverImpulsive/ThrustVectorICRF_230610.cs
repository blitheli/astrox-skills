    ...
    public partial class ManeuverImpulsiveTests
    {
        /*
         测试 Astrogator
            ...
            20230616 对比了机动矢量精度，与STK对比Earth Inertial下差别1e-14   Mulin LIU
            20230617 修改了算例及相应对比算例，令AbsError为1e-10，重新比较精度。Nulin LIU
            20231212    修改json输入,不再提供Propagators,引用缺省的积分器
        */
        [TestMethod()]
        public void Impulsive_ThrustVectorICRF_230610()
        {
            //  输入json文件的路径
            ...
            //  读取输入参数(json)
            string fp = Path.Combine(filePath0, "ThrustVectorICRF_230610.json");
            ...
            Assert.AreEqual(-4922227.7754377482779, output.Position.cartesianVelocity[id - 5], 2e-5);
            Assert.AreEqual(-2673542.5683194625890, output.Position.cartesianVelocity[id - 4], 2e-5);
            Assert.AreEqual(6480.0456511912685, output.Position.cartesianVelocity[id - 3], 2e-8);
            Assert.AreEqual(3704.1747301245982, output.Position.cartesianVelocity[id - 2], 2e-8);
            Assert.AreEqual(2013.7882695714442, output.Position.cartesianVelocity[id - 1], 2e-8);
