    ...
    public partial class ManeuverImpulsiveTests
    {
        /*
         测试 Astrogator
            ...
            结果与STK对比，位置精度～2e-4(m)，速度精度～2e-7 (m/s)
            20230609 初次创建                                           Mulin LIU
            20230614 修改了第一段积分段时间，使该算例与其他算例的积分时间一致。 Mulin LIU
            20230616 对比机动矢量（Moon Inertial）
                     计算的值：3.454995698536777     -5.572352733506932      2.6479973067340907
                    STK参考值：3.454995698884395     -5.572352733232083      2.647997306858769
                        偏差：4e-10
                                                                       Mulin LIU
        ...
        */
        [TestMethod()]
        public void Impulsive_ThrustVectorVNC_Moon_230609()
        {
            //  输入json文件的路径
            ...
            //  读取输入参数(json)
            string fp = Path.Combine(filePath0, "ThrustVectorVNC(Moon)_230608.json");
            ...
            Assert.AreEqual(1507598.4236847757529, output.Position.cartesianVelocity[id - 6], 2e-4);
            Assert.AreEqual(1293892.6060527787740, output.Position.cartesianVelocity[id - 5], 2e-4);
            Assert.AreEqual(350991.1978344991894, output.Position.cartesianVelocity[id - 4], 2e-4);
            Assert.AreEqual(-1031.9896084744420, output.Position.cartesianVelocity[id - 3], 2e-7);
            Assert.AreEqual(1124.3100160890551, output.Position.cartesianVelocity[id - 2], 2e-7);
            Assert.AreEqual(298.8331990619662, output.Position.cartesianVelocity[id - 1], 2e-7);
