...
namespace ASTROX.Astrogator.Tests
{
    public partial class ManeuverImpulsiveTests
    {
        /*
         测试 Astrogator
            ...
            20230616    比较机动矢量（Earth Inertial）
                        计算的值：3.772624357470704      -5.641312057230607      1.9856746285245208
                       STK参考值：3.895368795407998      -5.533090851144944      2.052073970581423
                        偏差较大，需要核对
                                                                           Mulin LIU
        ...
        */
        [TestMethod()]
        public void Impulsive_ThrustVectorVNC_Sun_230610()
        {
            //  输入json文件的路径
            ...
            //  读取输入参数(json)
            string fp = Path.Combine(filePath0, "ThrustVectorVNC(Sun)_230610.json");
            ...
            Assert.AreEqual(4455117.6733110169152, output.Position.cartesianVelocity[id - 6], 2e-5);
            Assert.AreEqual(-4370023.5578017354783, output.Position.cartesianVelocity[id - 5], 2e-5);
            Assert.AreEqual(-2373864.5775386980858, output.Position.cartesianVelocity[id - 4], 2e-5);
            Assert.AreEqual(5758.4550664049603, output.Position.cartesianVelocity[id - 3], 2e-8);
            Assert.AreEqual(4524.5351731683057, output.Position.cartesianVelocity[id - 2], 2e-8);
            Assert.AreEqual(2461.4968944071252, output.Position.cartesianVelocity[id - 1], 2e-8);
