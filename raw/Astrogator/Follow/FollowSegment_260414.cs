...
public partial class FollowTests
{
    ...
    //  20260414    初次创建
    [TestMethod()]
    public void Follow_DefaultJoiningAndSeparation_260414()
    {
        //  输入json文件的路径
        string filePath0 = new DirectoryInfo(Assembly.GetExecutingAssembly().Location).Parent.FullName;
        filePath0 = Path.Combine(filePath0, @"Astrogator/Follow");
        //  读取输入参数(json)
        string fp = Path.Combine(filePath0, "FollowSegment_260414.json");
    ...
    //  Follow段的测试，验证Joining和Separation设置都为Leader的结束
    //  20260414    初次创建
    [TestMethod()]
    public void Follow_AtEndJoiningAndSeparation_260414()
    {
        //  输入json文件的路径
        string filePath0 = new DirectoryInfo(Assembly.GetExecutingAssembly().Location).Parent.FullName;
        filePath0 = Path.Combine(filePath0, @"Astrogator/Follow");
        ...
        */
        double ebsl6 = 1e-6;
        Assert.AreEqual(0.0, output.MainSequenceResults[0].DurationSec, ebsl6);
        var rltSeg0 = output.MainSequenceResults[0];
        Assert.AreEqual(5596.6457930237393157, rltSeg0.FinalState.Cartesian.X * 0.001, ebsl6);
        Assert.AreEqual(-3201.9674254036199272, rltSeg0.FinalState.Cartesian.Y * 0.001, ebsl6);
        Assert.AreEqual(-1738.5264636795045590, rltSeg0.FinalState.Cartesian.Z * 0.001, ebsl6);
