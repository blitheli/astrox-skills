...
namespace AeroSpace.Libration.Tests
{
    ...
    public partial class LibrationBaseTests
    {
        ...
        //  标准值由stk转换提供: 20250801 04:00:00.000  L2点坐标系下的位置速度为
        //  (5000000,5000000,5000000,200,-200,100)
        //  国际单位制
        //  20221019 金杨
        [TestMethod()]     
        public void EML2Frame_Test02()
        {
            //调用JplDE430精密历表
            ...
            //设置转换时刻epoch
            JulianDate epoch = new JulianDate(new GregorianDate(2025, 8, 1, 4, 0, 0));
            ...
            //  输入测试参数(地心惯性系下的位置速度)
            //  由stk转换提供
            Cartesian rp = new Cartesian(-387403629.9922113539651, -243163348.4852312249132, -134233509.2709452728741);
            Cartesian vp = new Cartesian(375.1734487701852, -829.4672330610310, -329.3488200976326);
            ...
            //   与预设的标准值比对
            Assert.AreEqual(5000000.0, rv_emL2.Value[0], 5);
            Assert.AreEqual(5000000.0, rv_emL2.Value[1], 5);
            Assert.AreEqual(5000000.0, rv_emL2.Value[2], 5);

            Assert.AreEqual(200.0, rv_emL2.FirstDerivative[0], 0.001);
            Assert.AreEqual(-200.0, rv_emL2.FirstDerivative[1], 0.001);
            Assert.AreEqual(100.0, rv_emL2.FirstDerivative[2], 0.001);
            Assert.IsTrue(true);
